...
class ForgeMemoryTests(unittest.TestCase):
    ...
    def test_build_system_prompt_injects_specific_context(self):
        ...
        self.assertIn("identify the user's likely psychological state", prompt)
        self.assertIn("acknowledge it with exactly one natural sentence", prompt)
        self.assertIn("analysis paralysis", prompt)
        self.assertIn("People do not open an execution coach", prompt)
